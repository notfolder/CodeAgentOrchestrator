...
4. **ブランチマージ（2ステップ方式）**
   ...
   - `gitlab_client.create_merge_request(project_id, source_branch=selected_branch, target_branch=original_branch, title, description)`を呼び出し、内部MRを作成する
   - `gitlab_client.merge_merge_request(project_id, mr_iid=created_mr.iid)`を呼び出し、作成したMRをマージする（コンフリクト発生時はエラーを記録して処理を中断）
