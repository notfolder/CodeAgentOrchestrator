...
##### create_task_context_init() → TaskContextInitExecutor
...
##### create_agent(agent_config: AgentNodeConfig, prompt_config: PromptConfig, username: str, progress_reporter: ProgressReporter, env_id: str | None = None) → ConfigurableAgent
...
3. **User Config取得**
   - UserConfigClientからusernameのLLM設定を取得
...
##### create_strategy(task: Task, workflow_factory: WorkflowFactory | None = None, definition_loader: DefinitionLoader | None = None, task_repository: TaskRepository | None = None, issue_to_mr_converter: IssueToMRConverter | None = None) → ITaskStrategy
...
2. **Issueタイプの場合**
   ...
   - Trueの場合: issue_to_mr_converterとtask_repositoryをIssueToMRConversionStrategyに渡してインスタンスを生成して返す
   - Falseの場合: gitlab_client・config_manager・task_repositoryをIssueOnlyStrategyに渡してインスタンスを生成して返す

3. **MergeRequestタイプの場合**
   - workflow_factory・definition_loader・task_repositoryをMergeRequestStrategyに渡してインスタンスを生成して返す
...
WorkflowBuilderはグラフ定義のノード・エッジを受け取り、Agent FrameworkのWorkflowオブジェクトを組み立てる独立クラス。`create_workflow_from_definition()` の呼び出しごとに新規インスタンスを生成し、各ノード登録後に`build()`を呼び出す。
...
##### async get_context_value(ctx: WorkflowContext, key: str) → Any

**処理フロー**:

1. ctx.get_state(key)をawaitで呼び出し
2. 値を返す

##### async set_context_value(ctx: WorkflowContext, key: str, value: Any) → None

**処理フロー**:

1. ctx.set_state(key, value)をawaitで呼び出し

### 3.2 TaskContextInitExecutor

#### 3.2.1 概要

TaskContextInitExecutorはTaskContextの情報をワークフローコンテキストに転写する軽量な初期化Executorである。外部API呼び出しは行わず、MergeRequestStrategyが事前にキャッシュしたユーザー名・user_configをそのままコンテキストに保存する。
...
1. **TaskContextから情報取得**
   - msg（TaskContext）からusername、cached_user_config、project_id、mr_iid、issue_iidを取得

2. **ワークフローコンテキストに保存**
   - ctx.set_state('username', username)
   - ctx.set_state('user_config', cached_user_config)
   - ctx.set_state('project_id', project_id)
   - ctx.set_state('mr_iid', mr_iid)
   - ctx.set_state('issue_iid', issue_iid)
...
2. **Issueコメント取得**
   - gitlab_client.get_issue_notes(project_id, issue_iid)を呼び出し
   - コメント一覧を取得

3. **MRにコメント転記**
   - コメント一覧をループ
   - 各コメントについてgitlab_client.create_merge_request_note(project_id, mr_iid, comment.body)を呼び出し
...
4. **ブランチ直接マージ**
   - selected_branchがoriginal_branchと同一の場合はスキップする
   - `gitlab_client.merge_branch(project_id, source_branch=selected_branch, target_branch=original_branch)`を呼び出し、MRを作成せずにブランチを直接マージする（コンフリクト発生時はエラーを記録して処理を中断）
...
5. **コンテキスト圧縮チェック**
   - kwargsからGitLabユーザー名（username）を取得
   - compression_serviceが設定されており、usernameが非空文字列の場合のみ実行する
   - await ContextCompressionService.check_and_compress_async(task_uuid, username)を呼び出す（例外はキャッチしてログ記録し、主処理は継続する）
...
- **username: str** - GitLabユーザー名（設定取得用）
- **config: CompressionConfig** - 圧縮設定（ユーザー設定とモデル推奨値から構築）
- **system_config: Dict** - システムデフォルト設定とモデル推奨値マッピング

#### 4.4.3 主要メソッド

##### check_and_compress_async(task_uuid: str, username: str) → bool

**処理フロー**:

1. **ユーザー圧縮設定取得**
   - SQLクエリ実行: `SELECT context_compression_enabled, token_threshold, keep_recent_messages, min_to_compress, min_compression_ratio, model_name FROM user_configs WHERE username = ?`
...
##### save_token_usage(username: str, task_uuid: str, node_id: str, model: str, prompt_tokens: int, completion_tokens: int, total_tokens: int) → None
...
2. **リポジトリへの保存**
   - token_usage_repository.record_token_usage(username, task_uuid, node_id, model, prompt_tokens, completion_tokens)を呼び出してtoken_usageテーブルに記録する
...
#### async intercept(phase: str, node: WorkflowNode, context: WorkflowContext, **kwargs) → Optional[MiddlewareSignal]
...
5. **データベース記録**
   ...
   - username、task_uuid、node_id、model、prompt_tokens、completion_tokens、total_tokensを渡す
   - token_usageテーブルに保存

6. **メトリクス送信**
   - metrics_collector.send_metric()を呼び出し
   - metric_name='token_usage_total'
   - labels={'model': model, 'node_id': node_id, 'username': username}
   - value=total_tokens
...
- **current_todo_content: str | None** - 現在のTodoリストMarkdownテキスト（Todoがない場合はNone）
...
1. **ノード状態の更新**
   ...
   - `todo_changed`: node_statesは変更しない
...
2. **サマリ・応答の更新**
   ...
   - `todo_changed`: `current_todo_content = details["todo_markdown"]`（空文字の場合はNone）

3. **コメント上書き**
   - `comment_manager.update_progress_comment(context, mr_iid, node_states, latest_event_summary, latest_llm_response, error_detail, todo_content=current_todo_content)`を呼び出す
...
```
BaseExecutor（本システム共通Executor基底クラス）
...
### 11.4 handle(msg, ctx: WorkflowContext) の処理フロー
...
- Executor群: BaseExecutor、TaskContextInitExecutor、ContentTransferExecutor、PlanEnvSetupExecutor、ExecEnvSetupExecutor、BranchMergeExecutor
