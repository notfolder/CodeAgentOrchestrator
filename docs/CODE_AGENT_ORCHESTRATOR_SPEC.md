...
#### metadata.json のフィールド

- `task_uuid`: タスクUUID
- `total_file_reads`: ファイル読み込み総数
- `total_command_executions`: コマンド実行総数
- `total_mcp_calls`: MCPツール呼び出し総数
- `started_at`: 開始日時（ISO 8601形式）
- `last_updated_at`: 最終更新日時（ISO 8601形式）

#### ファイル読み込み結果のフィールド

- `timestamp`: 実行日時（ISO 8601形式）
- `tool`: 使用ツール名（text_editor等）
- `command`: コマンド種別（view等）
- `path`: 対象ファイルパス
- `content_preview`: 内容プレビュー（先頭部分）
- `content_length`: コンテンツ全体の長さ（バイト数）
- `mime_type`: MIMEタイプ

#### コマンド実行結果のフィールド

- `timestamp`: 実行日時（ISO 8601形式）
- `tool`: 使用ツール名（command-executor等）
- `command`: 実行コマンド
- `exit_code`: 終了コード
- `stdout`: 標準出力
- `stderr`: 標準エラー出力
