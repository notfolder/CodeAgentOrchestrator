...
コーディングエージェントを3種類の設定で並列実行する場合のエージェント定義。task_classifierからtest_execution_evaluationまでのエージェント定義は標準フローと仕様上共通だが、**実装上は全エージェント定義を独立して記述するフォーマットを採用する**（「継承」は論理的な仕様の共通性を指し、実装上のクラス継承機構ではない）。以下のエージェントを追加・差し替えする。
