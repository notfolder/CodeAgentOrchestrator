...
- Purpose: メールアドレスからユーザー設定を取得（LLM設定、コンテキスト圧縮設定、学習機能設定）
...
- Body: ユーザー情報（email、username、password、role、is_active）とLLM設定（圧縮設定は任意、未指定の場合はデフォルト値が適用される）、学習機能設定（learning_enabled、learning_llm_model、learning_llm_temperature、learning_llm_max_tokens、learning_exclude_bot_comments、learning_only_after_task_start）
- Validation: 
...
- Purpose: メールアドレスとパスワードで認証し、JWTアクセストークンを発行する
- Authentication: なし（認証不要）
- Body: `{"email": "...", "password": "..."}`
- Response: `{"access_token": "...", "token_type": "bearer", "expires_in": 86400}`
- エラー: メールアドレスまたはパスワードが不正な場合は HTTP 401 を返す
...
- Authentication: Bearer Token (Admin)
- Query: `user_email` (optional), `period` (日数, default 30)
- Response: `{"period_days": N, "user_email_filter": "...", "stats": [...]}`

**GET /api/v1/tasks**
- Purpose: タスク実行履歴一覧取得
- Authentication: Bearer Token (Admin)
- Query: `user_email` (optional), `status` (optional), `task_type` (optional), `page` (default 1), `per_page` (default 20, max 100)
